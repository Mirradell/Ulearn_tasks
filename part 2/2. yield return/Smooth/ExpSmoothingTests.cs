...
namespace yield
{
	...
	public class ExpSmoothingTests
	{
		...
		private void CheckSmooth(double alpha, double[] ys, double[] expectedYs)
		{
			...
			Assert.AreEqual(ys.Length, actual.Count, "Length");
			for (int i = 0; i < actual.Count; i++)
			{
				Assert.AreEqual(GetX(i), actual[i].X, 1e-7, "X");
				Assert.AreEqual(ys[i], actual[i].OriginalY, 1e-7, "Y");
				Assert.AreEqual(expectedYs[i], actual[i].ExpSmoothedY, 1e-7, "ExpSmoothed");
			}
		}
