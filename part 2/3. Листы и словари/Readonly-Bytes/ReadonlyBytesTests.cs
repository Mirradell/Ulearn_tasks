...
namespace hashes
{
    ...
    public class ReadonlyBytesTests
    {
        ...
        public void EqualOnSameBytes()
        {
            ...
            Assert.IsTrue(new ReadonlyBytes(new byte[0]).Equals(new ReadonlyBytes(new byte[0])), "1");
            Assert.IsTrue(new ReadonlyBytes(100).Equals(new ReadonlyBytes(100)), "2");
            Assert.IsTrue(new ReadonlyBytes(1, 2, 3).Equals(new ReadonlyBytes(1, 2, 3)), "3");
            var items = new ReadonlyBytes(4, 2, 67, 1);
            Assert.IsTrue(items.Equals(items), "4");
        ...
        {
            Assert.IsFalse(new ReadonlyBytes(bytes).Equals(null), "1");
            Assert.IsFalse(new ReadonlyBytes(bytes).Equals("string"), "2");
            Assert.IsFalse(new ReadonlyBytes(bytes).Equals(new DerivedFromReadonlyBytes()), "3");
        }

        [Test]
        public void HashCode()
        {
            Assert.AreEqual(new ReadonlyBytes().GetHashCode(), new ReadonlyBytes().GetHashCode(), "1");
            Assert.AreEqual(new ReadonlyBytes(1, 2, 3).GetHashCode(), new ReadonlyBytes(1, 2, 3).GetHashCode(), "2");
            Assert.AreNotEqual(new ReadonlyBytes(1, 2, 3).GetHashCode(), new ReadonlyBytes(1, 2, 3, 4).GetHashCode(), "3");
            Assert.AreNotEqual(new ReadonlyBytes(1, 2, 3).GetHashCode(), new ReadonlyBytes(1, 2, 4).GetHashCode(), "4");
            Assert.AreNotEqual(new ReadonlyBytes(1, 2, 3).GetHashCode(), new ReadonlyBytes(1, 2).GetHashCode(), "5");
            Assert.AreNotEqual(new ReadonlyBytes(1, 2, 3).GetHashCode(), new ReadonlyBytes(3, 2, 1).GetHashCode(), "6");
            Assert.AreNotEqual(new ReadonlyBytes(1, 0).GetHashCode(), new ReadonlyBytes(0, 31).GetHashCode(), "7");
            Assert.AreNotEqual(new ReadonlyBytes(0, 0).GetHashCode(), new ReadonlyBytes(255, 0).GetHashCode(), "8");
