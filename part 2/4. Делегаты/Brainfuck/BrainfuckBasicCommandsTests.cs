...
namespace func.brainfuck
{
	...
	public class BrainfuckBasicCommandsTests
	{
		...
		{
			Assert.AreEqual(2, Vm("<", 3).MemoryPointer, "1");
			Assert.AreEqual(0, Vm("<>", 3).MemoryPointer, "2");
			Assert.AreEqual(0, Vm(">>>", 3).MemoryPointer, "3");
			Assert.AreEqual(1, Vm(">>>>", 3).MemoryPointer, "4");
			Assert.AreEqual("\x1", Run("<+."), "5");
			Assert.AreEqual("\x2", Run("++<>."), "6");
			Assert.AreEqual("\x1", Run("+<++<+++>>."), "7");
			Assert.AreEqual("\x3", Run("+++" + new string('>', 30000) + "."), "8");
			Assert.AreEqual("\x3", Run("+++" + new string('<', 30000) + "."), "9");
		}
